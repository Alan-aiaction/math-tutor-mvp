...
function overallAccuracy(trend) {
...
export default function Dashboard({ accessToken, activeChild }) {
  ...
  const accuracy = overallAccuracy(heroKpis.accuracy_trend);
    ...
    <div className="flex w-full max-w-4xl flex-col gap-5">
      <section className="flex flex-col gap-1 rounded-2xl border border-border bg-bg p-6">
        ...
        <h1 className="font-display text-2xl font-bold text-ink">
          {t("dashboard.heroFor", { name: heroChild.nickname })}
        </h1>
      </section>

      <section className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <KpiCard label={t("dashboard.kpiAccuracy")} figure={formatPercent(accuracy)} />
        <KpiCard label={t("dashboard.kpiDays")} figure={heroKpis.practice_frequency_days} />
        <KpiCard label={t("dashboard.kpiRetries")} figure={heroKpis.average_retries.toFixed(1)} />
        <KpiCard label={t("dashboard.kpiTotal")} figure={heroKpis.total_attempts} />
      </section>
      ...
      <section className="rounded-2xl border border-border bg-bg p-6">
        ...
        <TrendChart trend={heroKpis.accuracy_trend} />
      </section>
      ...
      <section className="rounded-2xl border border-border bg-bg p-6">
        ...
        <div className="mt-3 flex flex-col gap-3">
          {heroKpis.weak_spots_by_topic.map((spot) => (
