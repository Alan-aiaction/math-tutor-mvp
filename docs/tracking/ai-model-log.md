...
| 2026-08-04 | Alan | Claude Code | Claude Sonnet 5 | `claude-sonnet-5` | Session covering: added a [Living] entry to decision-log.md for the recommended next-task sequence (dependency order, no owners), so it's readable directly rather than recomputed on request each time |
